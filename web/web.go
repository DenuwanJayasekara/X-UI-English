...
func (s *Server) startTask() {
	...
	go func() {
		...
		// Count traffic every 10 seconds, delay 5 seconds on first start, offset from xray restart time
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// Check inbound traffic overflow and expiry every 30 seconds
	s.cron.AddJob("@every 30s", job.NewCheckInboundJob())
	// Notify traffic stats once per day, Shanghai time 8:30
